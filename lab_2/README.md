...
As a first step, impute missing values for `annual_base_pay` using mean substitution. Then, based on all respondents that report a **non-zero (and non-NaN)** `annual_bonus`, calculate the average `annual_bonus`/`annual_base_pay` ratio. Finally, assume any respondent that didn't include `annual_bonus` information (i.e., is null) will actually receive that same ratio of their `annual_base_pay` as an annual bonus. As in Q6, compute the mean and standard deviation of `annual_bonus` after imputation among the survey respondents and submit your code as `q7.py` in the `submission/` directory; your code should write its output into `q7.csv` (also in the `submission/` directory), in the format:
```csv
imputed_mean,imputed_std
<mean>,<std>
```
