...
1. **Start Centralized:** You should be able to beat the `noop` and `silly-goose` (and possibly even `glossu-rabban`) rivals by writing an implementation in which one Fedaykin (the leader) is a centralized decision maker who decides which spice field(s) to destroy. Once the leader picks a field to destroy, it can use `fd = ray.get_actor(f"Fedaykin{some_id}")` to get references to the other Fedaykin and then call their `_ride_sandworm()` and `_destroy_spice_field()` methods *remotely*. Look out for race conditions, and be aware that **Actors can only execute a single method at a time in serial order.**
2. **Go Distributed One Step at a Time:** To beat `feyd-rautha`, you will (likely) need to write a distributed implementation in which each Fedaykin is acting independently for at least some portion of its `start()` method. **DO NOT TRY THE MOST COMPLICATED IMPLEMENTATION YOU CAN THINK OF FIRST.** Distributed programming is kryptonite for programmers who try to optimize their code too early. Start by implementing a simple form of message passing. Use print statements as needed to ensure that your messages are being sent and received as you would expect. You may want to set `RAY_DEDUP_LOGS: "0"` in your `runtime-env.yaml` and pipe your output to a file (e.g. `ray job submit ... -- python dune_game.py --rival noop > logs.txt`) so that you can use tools like `grep` and `sed` to parse your logs and check that things are working smoothly. Once you are confident that your Fedaykin are passing messages as expected, then test out your ideas in an incremental fashion.
